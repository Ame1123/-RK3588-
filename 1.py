...
def handle_ice_detection(detected_ice, temp, humi, auto_mode):
    ...
    """
    global icy_status, servo1_auto_rotation
    
    try:
        with deicing_lock:
            # 确定当前icy状态
            current_icy = 1 if detected_ice else 0
            
            # 如果icy状态发生变化，上报到云端
            if current_icy != icy_status:
                icy_status = current_icy
                publish_icy_status(mqtt_client_instance, icy_status)
            
            # 控制servo1旋转状态和servo2位置
            if detected_ice:
                # 检测到结冰，停止servo1自动旋转
                if servo1_auto_rotation:
                    print("检测到结冰，停止servo1自动旋转")
                    control_servo1_rotation(False)
            else:
                # 无冰状态，恢复servo1自动旋转，servo2回到0度
                if not servo1_auto_rotation:
                    print("检测无冰，恢复servo1自动旋转")
                    control_servo1_rotation(True)
                
                # servo2回到0度（每次检测无冰时都执行，确保servo2处于正确位置）
                print("检测无冰，servo2回到0度")
                reset_servo2_position()
            
            # 自动除冰逻辑：仅依靠摄像头检测结果
            # 当auto_mode为None时，默认当作自动模式处理
            if (auto_mode == 0 or auto_mode is None) and detected_ice:  # 自动模式且检测到结冰
                print("摄像头检测到结冰，启动除冰程序")
                trigger_deicing()
                
                # 注释掉原有的环境条件检测逻辑
                # if check_icing_conditions(temp, humi):  # 满足结冰条件
                #     print("检测到结冰且满足除冰条件，启动除冰程序")
                #     trigger_deicing()
                # else:
                #     print("检测到结冰但环境条件不满足除冰要求")

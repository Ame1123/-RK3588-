...
# 限制 ONNX Runtime / OpenMP 使用的CPU核心数，避免推理时把所有核占满，
# 导致内核来不及处理USB中断而把摄像头disconnect
...
ICE_TRIGGER_THRESHOLD = 5  # 5帧中至少有这么多帧检测到结冰才认定（容忍偶发漏检）
current_servo2_position = 0  # 当前servo2位置：0=0度，45=45度

# 除冰处理工作队列（队列大小1，新数据覆盖旧数据，避免堆积）
import queue
ice_event_queue = queue.Queue(maxsize=1)

# RTP发送队列（只保留最新一帧，避免堆积+RTP发送阻塞主循环）
...
def handle_ice_detection(detected_ice, temp, humi, auto_mode):
    ...
    try:
        ...
        # 如果缓冲区还没有满，继续收集数据（不打印，减少stdout压力）
        if len(ice_detection_buffer) < CONSECUTIVE_CHECKS:
            return
        ...
        if auto_mode == 1:
            ...
            if ice_count >= ICE_TRIGGER_THRESHOLD:
                print(f"手动模式 - 检测到结冰 ({ice_count}/{CONSECUTIVE_CHECKS})")
                publish_icy_status(mqtt_client_instance, 1)
            else:
                print(f"手动模式 - 未达到结冰阈值 ({ice_count}/{ICE_TRIGGER_THRESHOLD})，按无冰处理")
        ...
        with deicing_lock:
            ...
            # 5帧中达到阈值即触发除冰（容忍偶发漏检）
            if ice_count >= ICE_TRIGGER_THRESHOLD:
                print(f"自动模式 - 检测到结冰 ({ice_count}/{CONSECUTIVE_CHECKS})")
...
def publish_from_serial(client):
    """从串口读取数据并发布到MQTT"""
    global current_auto  # 必须声明，否则 current_auto = data['auto'] 会创建局部变量
    ...
    while running:
        try:
            ...
            elif ('servo1' in data or 'servo2' in data or 'servo3' in data or 'servo4' in data or 'auto' in data or 'switch' in data or 'box' in data):
                properties = {}
                # 添加servo相关数据
...
def load_yolo_model(model_path):
    ...
    try:
        ...
        if not os.path.exists(onnx_path):
            print("导出ONNX模型...")
            YOLO(model_path).export(format="onnx", opset=12, dynamic=False, simplify=True)
        print("加载ONNX模型（onnxruntime加速）...")
        ...
        try:
            if camera_cap is None or not camera_cap.isOpened():
                ...
                if camera_cap.isOpened():
                    ...
                    time.sleep(2)  # 设备真不存在时不要狂试
                continue
            # 关键：丢弃V4L2缓冲区里的积压帧，只保留最新一帧，
            # 否则推理慢于摄像头帧率会让USB驱动持续报错最终 disconnect
            for _ in range(2):
                camera_cap.grab()
            ret, frame = camera_cap.retrieve()
            if not ret:
                fail_count += 1
                ...
                if fail_count >= 3:
                    try:
                        ...
                        pass
                    camera_cap = None  # 让外层循环走重新打开分支
                    fail_count = 0
                else:
                    time.sleep(0.1)
                continue
            fail_count = 0  # 读取成功，重置失败计数

            # YOLO检测
            results = yolo_model(frame, verbose=False)
            has_box = len(results[0].boxes) > 0

            # 用cv2直接画框，避开ultralytics的plot()——它会触发PIL字体加载，
            # 在RK3588上会导致单帧耗时飙升、V4L2缓冲堆积，最终USB摄像头被驱动disconnect
            ...
            if has_box:
                ...
                for box in results[0].boxes:
                    try:
                        ...
                        color = (0, 0, 255) if names.get(cls_id) == ICE_DETECTION_CLASS else (0, 255, 0)
            ...
            frame_count += 1
            if frame_count % 100 == 0:  # 100帧才打一次（约5秒）
                print(f"[vision] 已处理 {frame_count} 帧")

            # 注意：cv2.imshow 不能在子线程调用（会导致摄像头卡死），如需本地预览请通过RTP流查看
                    ...
                    if hasattr(box, 'cls') and box.cls is not None:
                        ...
                        if hasattr(yolo_model, 'names') and class_id in yolo_model.names:
                            ...
                            if class_name == ICE_DETECTION_CLASS:
                                detected_ice = True
                                break
                # 每帧打印一次改成只在结冰状态变化时打印（大幅减少打印量）

            # 将除冰处理放入队列由单独工作线程处理，避免串口/MQTT阻塞摄像头循环
            try:
                ice_event_queue.put_nowait((detected_ice, current_temp, current_humi, current_auto))
            except queue.Full:
                # 队列满说明上一次还没处理完，丢弃当前帧的事件即可（缓冲区会继续累积）
                pass

            # 不需要额外sleep，推理本身就是限速器，sleep反而让V4L2缓冲区堆积导致USB掉线
...
def main():
    ...
    # 启动除冰处理工作线程（消费检测事件队列）
    ice_worker = threading.Thread(target=ice_worker_thread, daemon=True)
    ice_worker.start()

    # 启动RTP发送工作线程（把编码+UDP发送从主循环剥离，防止阻塞摄像头采集）
    rtp_thread = threading.Thread(target=rtp_sender_thread, daemon=True)
    rtp_thread.start()

    ...
    try:
        ...
        with deicing_lock:
            ...
            # 情况3：混合结果，继续收集下一轮数据
                ...
                print(f"检测结果混合，继续收集数据 (结冰={ice_count}次, 无冰={no_ice_count}次)")
                
                # 混合结果时的处理逻辑：
                # 如果无冰次数占多数且servo1已停止，恢复旋转
                if no_ice_count > ice_count and not servo1_auto_rotation:
                    print("无冰次数占多数，恢复servo1旋转")
                    # servo2回到0度
                    print("确保servo2回到0度位置")
                    control_servo2_position(0)
                    # 添加延时确保命令处理完成
                    time.sleep(0.1)
                    # 然后恢复servo1旋转
                    control_servo1_rotation(True)
                # 如果结冰次数占多数且servo1在旋转，停止旋转
                elif ice_count > no_ice_count and servo1_auto_rotation:
                    print("结冰次数占多数，停止servo1旋转")
                    control_servo1_rotation(False)
                    # 添加延时确保命令处理完成
                    time.sleep(0.1)
                    # 确保servo2处于45度位置
                    print("确保servo2处于45度位置")
                    control_servo2_position(45)
                # 如果结冰和无冰次数相等，根据当前状态决定
                elif ice_count == no_ice_count:
                    print("结冰和无冰次数相等")
                    # 如果当前servo1已停止且检测到更多无冰，倾向于恢复旋转
                    if not servo1_auto_rotation:
                        print("servo1已停止，倾向于恢复旋转")
                        print("确保servo2回到0度位置")
                        control_servo2_position(0)
                        # 添加延时确保命令处理完成
                        time.sleep(0.1)
